...
// StoE converts a Go UTF-8 string to an EBCDIC CP37 byte array. Only Unicode
// code points <=0xFF are translated, anything else will be converted to an
// EBCDIC space (0x40).
func StoE(s string) []byte {
	...
	for i, r := range runes {
		...
		output[i] = ucToCP037[r]
	}

	return output
}

// EtoS converts an EBCDIC CP37 byte array to a Go UTF-8 string.
func EtoS(e []byte) string {
	output := make([]rune, len(e))
	for i, b := range e {
		output[i] = cp037toUC[b]
	}
