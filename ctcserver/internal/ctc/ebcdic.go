...
const (
	CodepageCP37    Codepage = "cp37"
	CodepageBracket Codepage = "bracket"
)

func SetGlobalCodepage(cp Codepage) {
	switch cp {
	case CodepageCP37:
		activeE2U = cp037toUC
		activeU2E = ucToCP037
	case CodepageBracket:
		activeE2U = bracketToUC
		activeU2E = ucToBracket
	default:
		log.Error().Msgf("Unknown code page %s. No change made.", cp)
	}
}

// CP037<->Unicode mapping based on the data in
// ftp://ftp.unicode.org/Public/MAPPINGS/VENDORS/MICSFT/EBCDIC/CP037.TXT

var cp037toUC []rune = []rune{
...
// These tables are the same as the above CP037<->Unicode tables, except
// for the square bracket [ ] characters. x3270 and friends, and Vista3270
// use this alternate mapping by default.
//
// https://x3270.miraheze.org/wiki/Why_are_the_square_bracket_characters_displayed_wrong%3F
var bracketToUC []rune = []rune{
	0x00, 0x01, 0x02, 0x03, 0x9c, 0x09, 0x86, 0x7f, // 0x00..0x07
	0x97, 0x8d, 0x8e, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, // 0x08..0x0f
	0x10, 0x11, 0x12, 0x13, 0x9d, 0x85, 0x08, 0x87, // 0x10..0x17
	0x18, 0x19, 0x92, 0x8f, 0x1c, 0x1d, 0x1e, 0x1f, // 0x18..0x1f
	...
	0x71, 0x72, 0xaa, 0xba, 0xe6, 0xb8, 0xc6, 0xa4, // 0x98..0x9f
	0xb5, 0x7e, 0x73, 0x74, 0x75, 0x76, 0x77, 0x78, // 0xa0..0xa7
	0x79, 0x7a, 0xa1, 0xbf, 0xd0, 0x5b, 0xde, 0xae, // 0xa8..0xaf
	0x5e, 0xa3, 0xa5, 0xb7, 0xa9, 0xa7, 0xb6, 0xbc, // 0xb0..0xb7
...
var ucToBracket []byte = []byte{
	...
	0xc8, 0xc9, 0xd1, 0xd2, 0xd3, 0xd4, 0xd5, 0xd6, // 0x48..0x4f
	0xd7, 0xd8, 0xd9, 0xe2, 0xe3, 0xe4, 0xe5, 0xe6, // 0x50..0x57
	0xe7, 0xe8, 0xe9, 0xad, 0xe0, 0xbd, 0xb0, 0x6d, // 0x58..0x5f
	0x79, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0x87, // 0x60..0x67
	0x88, 0x89, 0x91, 0x92, 0x93, 0x94, 0x95, 0x96, // 0x68..0x6f
	0x97, 0x98, 0x99, 0xa2, 0xa3, 0xa4, 0xa5, 0xa6, // 0x70..0x77
	0xa7, 0xa8, 0xa9, 0xc0, 0x4f, 0xd0, 0xa1, 0x07, // 0x78..0x7f
	...
	0x38, 0x39, 0x3a, 0x3b, 0x04, 0x14, 0x3e, 0xff, // 0x98..0x9f
	0x41, 0xaa, 0x4a, 0xb1, 0x9f, 0xb2, 0x6a, 0xb5, // 0xa0..0xa7
	0xbb, 0xb4, 0x9a, 0x8a, 0x5f, 0xca, 0xaf, 0xbc, // 0xa8..0xaf
	0x90, 0x8f, 0xea, 0xfa, 0xbe, 0xa0, 0xb6, 0xb3, // 0xb0..0xb7
	0x9d, 0xda, 0x9b, 0x8b, 0xb7, 0xb8, 0xb9, 0xab, // 0xb8..0xbf
	0x64, 0x65, 0x62, 0x66, 0x63, 0x67, 0x9e, 0x68, // 0xc0..0xc7
	0x74, 0x71, 0x72, 0x73, 0x78, 0x75, 0x76, 0x77, // 0xc8..0xcf
...
// Set to brackets mapping by default
var activeE2U = bracketToUC
var activeU2E = ucToBracket

// StoE converts a Go UTF-8 string to an EBCDIC  byte array. Only Unicode code
// points <=0xFF are translated, anything else will be converted to an EBCDIC
// space (0x40).
func StoE(s string) []byte {
	runes := []rune(s)
	output := make([]byte, len(runes))
	for i, r := range runes {
		if r > 0xFF {
			// outside our mappable characters. Replace with a space.
			output[i] = 0x40
			continue
		}

		output[i] = activeU2E[r]
	}

	return output
}

// EtoS converts an EBCDIC byte array to a Go UTF-8 string.
func EtoS(e []byte) string {
	output := make([]rune, len(e))
	for i, b := range e {
		output[i] = activeE2U[b]
	}
